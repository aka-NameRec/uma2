...
# (20260203-033058) AGENTS.md

Deduplicated, stack-only rules for Codex agents. Follow the most specific rule when conflicts arise.

---

## General Engineering Principles

- Prefer small, coherent changes; avoid unrelated cleanup.
- Preserve existing behavior unless explicitly requested.
- Verify information; do not speculate.
- Use meaningful names; avoid unclear abbreviations.
- Replace magic values with named constants.
- Keep functions small and single-purpose.
- Keep related code together; maintain a clean structure.
- Use comments only to explain intent or non-obvious decisions.
- Follow DRY; extract reusable logic rather than duplicating.
- Refactor early when debt appears; leave code cleaner than found.
- Write tests before fixing bugs; cover edge cases and error conditions.

## Agent Response Rules

- Make changes file by file.
- Provide edits in a single chunk per file.
- Do not invent changes not explicitly requested.
- Do not remove unrelated code or functionality.
- Do not ask for confirmation of already-provided context.
- Do not suggest whitespace-only changes.
- Do not discuss current implementation unless asked.
- Do not add apologies or "understanding" feedback.
- Do not summarize changes made.
- Provide real file paths when referencing files.

<!-- ## Gitflow (When Used)

- Never commit directly to `main`, `master`, or `develop`.
- Use `feature/*`, `release/*`, and `hotfix/*` branches with descriptive names.
- Commit message format: `type(scope): description`.
- Use semantic versioning for releases.
- All changes go through PRs; CI must pass before merge.
- Protected branches require reviews, up-to-date branches, and no force pushes. -->

## Context7 MCP Usage

- When library/API docs or setup steps are needed, use Context7 MCP automatically instead of relying on memory.
- Add a rule in the MCP client to auto-invoke Context7 to avoid typing `use context7` every time.
- If you know the exact library, include the Context7 library ID in the prompt using slash syntax (e.g., `/supabase/supabase`) to skip resolution.
- Specify the library version in the prompt when version-specific behavior matters.

## ConPort MCP Usage (VS Code / Codex)
...
- Use Django built-ins whenever possible; follow PEP 8.
- Organize code into Django apps for separation of concerns.
- Keep views thin and focused on request handling.
- Prefer CBVs for complex views; FBVs for simple views.
- Keep business logic in models/forms; views remain thin.
- Use DRF serializers for JSON; Django templates for HTML.
- Define clear, RESTful URL patterns.
- Use the ORM; avoid raw SQL unless required for performance.
- Use `select_related` and `prefetch_related` to avoid N+1 queries.
- Index and optimize queries; prefer ORM/SQL aggregations over Python loops.
- Use caching (Redis/Memcached) where appropriate.
- Use async views or Celery for long-running or I/O-bound tasks.
- Wrap multi-step writes in `transaction.atomic()`.
- Do not perform network calls inside transactions.
- Use timezone-aware datetimes only.
- Apply Django security best practices (CSRF, XSS, SQL injection protection).
- Review migrations manually; avoid locking large tables without a plan.

## Frontend — Nuxt 3 + Vue 3

- Use the Composition API; keep `setup()` clean.
- Keep components small and focused; minimize template logic.
- Use proper props validation and explicit emits.
- Use `ref`, `reactive`, `computed`, `watch`, and lifecycle hooks appropriately.
- Prefer composables for reusable logic.
- Use Pinia for state management; keep stores modular.
- Handle async state and loading/error states explicitly.
- Use Vue Router properly; leverage route meta and lazy loading.
- Use `v-model` correctly; validate forms; reset state cleanly.
- Use `v-show` vs `v-if` intentionally; avoid unnecessary watchers.
- Implement component lazy loading and caching when beneficial.
- Use Tailwind classes for styling; avoid `<style>` tags.
- Use early returns to improve readability.
- Use descriptive names; event handlers use a `handle*` prefix.
- Prefer `const` arrow functions (e.g., `const toggle = () =>`).
- Fully implement requested functionality and verify thoroughly.

## Frontend — TypeScript in Vue

- Type components, props, emits, composables, and stores.
- Use proper prop types and emits definitions.
- Keep type definitions close to usage or in a `types` module.
...
- Provide alt text for images and labels for inputs.
- Ensure sufficient color contrast and `prefers-reduced-motion` support.
- Make interactive elements keyboard reachable and operable.
- Use `aria-live` for dynamic error messaging.
- Ensure correct landmarks and reading order.
- Test with screen readers; use tools like axe-core.

## TypeScript (Shared)

- Prefer interfaces for object shapes; use types for unions/intersections.
- Avoid `any`; prefer `unknown` when necessary.
- Enable strict type checking.
- Use explicit return types for public functions.
- Use generics and utility types for reusable typing.
- Use discriminated unions and type guards for runtime validation.
- Use readonly for immutable properties.
- Avoid type assertions unless necessary.

## Chrome Extensions — TypeScript (Manifest V3)

- Use MV3 with service workers unless a justified exception exists.
- Keep a clear separation between background, content, popup, and options.
- Use message passing for cross-context communication.
- Persist state in `chrome.storage`; do not rely on in-memory service worker state.
- Use least-privilege permissions; prefer optional permissions.
- Implement CSP in `manifest.json`.
- Use HTTPS for network requests.
- Sanitize and validate all external inputs.
- Handle API errors explicitly for all `chrome.*` calls.
- Use `chrome.alarms` for scheduled work instead of `setInterval`.
- Use TypeScript interfaces for message payloads and API responses.
- Use union types and type guards for message validation.
- Avoid logging secrets or sensitive page content.
- Follow official Chrome Extension documentation and best practices.

## Chrome Extensions — Code Output

- When providing code, output the entire file content, not just new or modified parts.
- Include all necessary imports, declarations, and surrounding code.
- Add comments for significant changes or additions.
